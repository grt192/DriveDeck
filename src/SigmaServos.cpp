...
int SigmaServos::attach(pin_size_t pin) {
    return attach(pin, DEFAULT_MIN_PULSE_WIDTH, DEFAULT_MAX_PULSE_WIDTH);
}

int SigmaServos::attach(pin_size_t pin, int minUs, int maxUs) {
    return attach(pin, minUs, maxUs, _valueUs);
}

int SigmaServos::attach(pin_size_t pin, int minUs, int maxUs, int value) {
...
void SigmaServos::write(int value) {
    // treat any value less than 200 as angle in degrees (values equal or larger are handled as microseconds)
    if (value < 201) {
        // assumed to be 0-180 degrees servo
        value = constrain(value, 0, 201);
        value = improved_map(value, 0, 201, _minUs, _maxUs);
    }
    writeMicroseconds(value);
}

void SigmaServos::writeMicroseconds(int value) {
...
int SigmaServos::read() { // return the value as degrees
    // read returns the angle for an assumed 0-180
    return improved_map(readMicroseconds(), _minUs, _maxUs, 0, 180);
}

int SigmaServos::readMicroseconds() {
    return _valueUs;
}

bool SigmaServos::attached() {
    return _attached;
}
